...
class ArticleInWechatFragment private constructor() : BaseFragment<FragmentArticlesBinding>(FragmentArticlesBinding::inflate),
    ...
    private fun pageHomeArticle() {
        flowRequest<ApiResponse.Paging<Article>> {
            request {
                ...
                RetrofitHelper.getInstance().create(Api::class.java).listArticleInWechat(mPage, mWechatId.orEmpty())
            }
            response {
                if (it.isSuccess) {
