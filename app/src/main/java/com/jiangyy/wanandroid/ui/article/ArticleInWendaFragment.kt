...
class ArticleInWendaFragment private constructor() : BaseFragment<FragmentArticlesBinding>(FragmentArticlesBinding::inflate),
    ...
    private fun pageHomeArticle() {
        flowRequest<ApiResponse.Paging<Article>> {
            request {
                ...
                RetrofitHelper.getInstance().create(Api::class.java).listWenda(mPage)
            }
            response {
                if (it.isSuccess) {
