...
class ArticleInSquareFragment private constructor() : BaseFragment<FragmentArticlesBinding>(FragmentArticlesBinding::inflate),
    ...
    private fun pageHomeArticle() {
        flowRequest<ApiResponse.Paging<Article>> {
            request {
                ...
                RetrofitHelper.getInstance().create(Api::class.java).listSquare(mPage)
            }
            response {
                if (it.isSuccess) {
