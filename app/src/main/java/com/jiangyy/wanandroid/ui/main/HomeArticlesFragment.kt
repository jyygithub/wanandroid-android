...
class HomeArticlesFragment : BaseFragment<FragmentArticlesBinding>(FragmentArticlesBinding::inflate),
    ...
    private fun pageHomeArticle() {
        flowRequest<ApiResponse.Paging<Article>> {
            request {
                ...
                RetrofitHelper.getInstance().create(Api::class.java).pageHomeArticle(mPage)
            }
            response {
                if (it.isSuccess) {
