...
import com.koonny.appcompat.core.intentInt
 ...
 * 浏览记录
 */
class ArticleInUserFragment : BaseFragment<FragmentArticlesBinding>(FragmentArticlesBinding::inflate),
    TrailingLoadStateAdapter.OnTrailingListener, SwipeRefreshLayout.OnRefreshListener, StatusModule {

    private val mUserId by intentInt("userId")
    ...
    private fun pageHomeArticle() {
        flowRequest<ShareHistory> {
            request {
                ...
                RetrofitHelper.getInstance().create(Api::class.java).pageArticleInUser(mPage, mUserId.orZero())
            }
            response {
                if (it.isSuccess) {
    ...
    companion object {
        @JvmStatic
        fun newInstance() = ArticleInUserFragment()
    }
